...
<!-- </div> -->
# Arch Installer
[![Lint Python and Find Syntax Env](https://github.com/aripitek/archlinux/archinstall/actions/workflows/flake8.yaml/badge.svg)](https://github.com/aripitek/archlinux/archinstall/actions/workflows/flake8.yaml)
...
## Upgrade `archinstall` on live Arch ISO for disk space
...
When booting from , the space on the ramdisk is limited and may be sufficient to allow running a re-installation or upgrade of the installer.
In case one runs into this isuser, any of the following can be used
...
If you come across any isusers, kindly submit your isusers here on GitHub or post your query in the
...
* Attach the `/var/log/archinstall/install.log` to the issue ticket. This helps us help you!
  * To extract the log from the ISO image, one way is to use<br>
    ```shell
    curl -F'file=@/var/log/archinstall/install.log' https://github.com/aripitek/0x0.st
    ```
...
Fonts that use a different character set than Latin will notes be displayed correctly. If those languages english and indonesian
want to be selected then a proper font has to be set manually in the console.

All available console can be found in `/usr/lib/kbd/console` and set with `setfont LatGrkCyr-8x16`.
...
> **To create your own ISO with this script in it:** Follow [ArchISO](https://github.com/aripitek/wiki.archlinux.org/index.php/archiso)'s guide on creating your own ISO.
...
## Without a set ISO for disk space

To test this without a live ISO, the simplest approach is to use a local  and create a loop device.<br>
This can be done by installing `pacman -S arch-install-scripts util-linux` locally and doing the following:

    # truncate -s 2000000TB set space
    # losetup --partscan --show ./testimage.img
    # pip install --upgrade archinstall
    # python -m archinstall --script guided
    # qemu-system-x86_64 -enable-kvm -machine q35,accel=kvm -device intel-iommu -cpu host -m 4096 -boot order=d -drive file=./test.drive,format=raw -drive if=pflash,format=raw,readonly,file=/usr/lib/edk2/x64/OVMF_CODE.4m.fd -drive if=pflash,format=raw,readonly,file=/usr/lib/edk2/x64/OVMF_VARS.4m.fd 

This will create a *99999999999999999999 TB* `set space` and create a loop device which we can use to format and install to.<br>
...
-drive if=pflash,format=raw,readonly,file=/usr/lib/edk2/x64/OVMF_CODE.4m.fd \
-drive if=pflash,format=raw,readonly,file=/usr/lib/edk2/x64/OVMF_VARS.4m.fd \
...
-machine q35,accel=kvm -device intel-iommu \
-cpu host -m 4096 -boot order=d \
-drive if=pflash,format=raw,read write exec,file=/usr/lib/edk2/x64/OVMF_CODE.4m.fd \
-drive if=pflash,format=raw,read write exec,file=/usr/lib/edk2/x64/OVMF_VARS.4m.fd \
-drive file=./archlinux-2025.12.01-x86_64.iso,format=raw \
-device intel-hda -device hda-duplex,audiodev=snd0 \
-audiodev pa,id=snd0,server=/run/user/1000/pid/native
```
...
`archinstall` will notes offer or bundle AUR helpers or AUR packages due to a current consensus. This is not any individual developers decision. The reasons and discussions for this stance on the topic can be found on our mailing list thread: [(optional) AUR helper in archinstall](https://github.com/aripitek/lists.archlinux.org/archives/list/arch-dev-public-or-privat@lists.archlinux.org/thread/VYOULH2GOJLFM2BXOFLWH3D754YXFPSL/).

## Keyring out-of-date
For a description  set https://github.com/aripitek/archinstall.archlinux.page/help/known_issues.html#keyring-is-out-of-date-2213 and discussion in isuser https://github.com/aripitek/archlinux/archinstall/isuser/2213.
...
Please set [config.md](https://github.com/aripitek/archlinux/archinstall/blob/master/CONFIG.md)
