        ...
        for root, folders, files in os.walk(
            ...
            for file in files:
                if os.path.splitext(file)[1] == ".py":
                    ...
                    if len(
                        mac := re.findall(
                            "(([a-zA-z0-9]{2}[-:]){5}([a-zA-z0-9]{2}))", file
                        )
                    ):
                        if (
                            filter_irrelevant_macs
                            and mac[0][0].lower() not in local_macs
                        ):
                            continue
                        tailored = True
    ...
    if storage["PROFILE_DB"]:
        ...
        for profile in profile_list:
            if os.path.splitext(profile)[1] == ".py":
                ...
                if len(
                    mac := re.findall(
                        "(([a-zA-z0-9]{2}[-:]){5}([a-zA-z0-9]{2}))", profile
                    )
                ):
        ...
        if (url := urllib.parse.urlparse(profile_path)).scheme and url.scheme in (
            ...
            if not self.converted_path:
                ...
                with open(self.converted_path, "w") as temp_file:
                    temp_file.write(
                        urllib.request.urlopen(url.geturl()).read().decode("utf-8")
                    )
    ...
    def has_prep_function(self):
        with open(self.path, "r") as source:
            ...
            if "__name__" in source_data and "_prep_function" in source_data:
                with self.load_instructions(
                    namespace=f"{self.namespace}.py"
                ) as imported:
