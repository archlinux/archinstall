...
def mnu_set_root_password(menu :archinstall.GlobalMenu) -> str:
...
def mnu_select_harddrives(menu :archinstall.GlobalMenu) -> Optional[str]:
	...
	if not harddrives:
		...
		if choice == 'no':
			return mnu_select_harddrives(menu)

	return harddrives

def mnu_select_profile(menu :archinstall.GlobalMenu) -> Optional[str] :
...
def mnu_create_superuser_account(menu :archinstall.GlobalMenu) -> str:
	superuser = archinstall.ask_for_superuser_account('Create a required super-user with sudo privileges: ', forced=True)
	return superuser

def mnu_create_user_account(menu :archinstall.GlobalMenu) -> List[Dict[str, Dict[str, str]]]:
...
def mnu_missing_configs(menu :archinstall.GlobalMenu) -> int:
	def check(s):
...
def mnu_install_text(menu :archinstall.GlobalMenu) -> str:
	missing = mnu_missing_configs(menu)
	if missing > 0:
		return f'Install ({missing} config(s) missing)'
	return 'Install'

def mnu_update_install(menu :archinstall.GlobalMenu):
	text = mnu_install_text(menu)
	menu.option('install').update_description(text)

def mnu_post_callback(menu :archinstall.GlobalMenu,option,value :Any = None):
	mnu_update_install(menu)
...
def define_base_option_set(menu :archinstall.GlobalMenu):
	...
	menu.set_option('harddrives',
			archinstall.Selector(
				'Select harddrives',
				lambda: mnu_select_harddrives(menu)))
	...
	menu.set_option('!superusers',
			archinstall.Selector(
				...
				lambda: mnu_create_superuser_account(menu),
				dependencies_not=['!root-password'],
				display_func=lambda x: list(x.keys()) if x else ''))
	menu.set_option('!users',
			archinstall.Selector(
				'Specify user account',
				lambda: mnu_create_user_account(menu),
				default={},
				display_func=lambda x: list(x.keys()) if x else '[]'))
	menu.set_option('profile',
			archinstall.Selector(
				'Specify profile',
				lambda: mnu_select_profile(menu),
...
def define_base_action_set(menu :archinstall.GlobalMenu):
	...
	menu.set_option('install',
			archinstall.Selector(
				mnu_install_text(menu),
				exit_func=lambda: True if mnu_missing_configs(menu) == 0 else False,
				enabled=True))
	menu.set_option('abort', archinstall.Selector('Abort', exit_func=lambda: exit(1), enabled=True))
