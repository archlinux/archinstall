...
class BlockDevice():
    ...
    def device(self):
        ...
        if self.info['type'] == 'loop':
            ...
            log(
                f"Unknown blockdevice type for {self.path}: {self.info['type']}", level=LOG_LEVELS.Debug)
    ...
    def partitions(self):
        ...
        if not o[:1] == b'{':
            raise DiskError(f'Error getting JSON output from:',
    ...
    @property
    def uuid(self):
        log(f'BlockDevice().uuid is untested!',
            level=LOG_LEVELS.Warning, fg='yellow')
        """
...
class Partition():
    ...
    def encrypted(self, value: bool):
        if value:
            log(f'Marking {self} as encrypted: {value}',
                level=LOG_LEVELS.Debug)
            log(
                f"Callstrack when marking the partition: {''.join(traceback.format_stack())}", level=LOG_LEVELS.Debug)
    ...
    def detect_inner_filesystem(self, password):
        log(
            f'Trying to detect inner filesystem format on {self} (This might take a while)', level=LOG_LEVELS.Info)
    ...
    def safe_to_format(self):
        if self.allow_formatting is False:
            log(f"Partition {self} is not marked for formatting.",
                level=LOG_LEVELS.Debug)
            return False
        elif self.target_mountpoint == '/boot':
            try:
                if self.has_content():
                    log(f"Partition {self} is a boot partition and has content inside.",
                        level=LOG_LEVELS.Debug)
                    return False
    ...
    def encrypt(self, *args, **kwargs):
        ...
        if not self.safe_to_format():
            log(f"Partition {self} was marked as protected but encrypt() was called on it!",
                level=LOG_LEVELS.Error, fg="red")
            return False
...
class Filesystem():
    ...
    def __enter__(self, *args, **kwargs):
        ...
        # TODO: partition_table_type is hardcoded to GPT at the moment. This has to be changed.
            ...
            log(f'Kept partition format {self.mode} for {self.blockdevice}',
                level=LOG_LEVELS.Debug)
        else:
            raise DiskError(
    ...
    def raw_parted(self, string: str):
        ...
        log(f"'parted -s {string}' returned: {b''.join(x)}",
            level=LOG_LEVELS.Debug)
        return x
    ...
    def use_entire_disk(self, root_filesystem_type='ext4'):
        log(f"Using and formatting the entire {self.blockdevice}.",
            level=LOG_LEVELS.Debug)
        self.add_partition('primary', start='1MiB',
                           end='513MiB', format='fat32')
        self.set_name(0, 'EFI')
        self.set(0, 'boot on')
        ...
        log(
            f"Set the root partition {self.blockdevice.partition[1]} to use filesystem {root_filesystem_type}.", level=LOG_LEVELS.Debug)
    ...
    def add_partition(self, type, start, end, format=None):
        ...
        if format:
            partitioning = self.parted(
                f'{self.blockdevice.device} mkpart {type} {format} {start} {end}') == 0
        else:
            partitioning = self.parted(
                f'{self.blockdevice.device} mkpart {type} {start} {end}') == 0
...
def get_partitions_in_use(mountpoint):
    try:
        output = b''.join(sys_command(
            f'/usr/bin/findmnt --json -R {mountpoint}'))
    except SysCallError:
        return {}
