    ...
    def device(self):
        ...
        if self.info["type"] == "loop":
            for drive in json.loads(
                b"".join(sys_command(["losetup", "--json"], hide_from_log=True)).decode(
                    "UTF_8"
                )
            )["loopdevices"]:
        ...
        if len(r["blockdevices"]) and "children" in r["blockdevices"][0]:
            ...
            for part in r["blockdevices"][0]["children"]:
                ...
                if part_id not in self.part_cache:
                    ...
                    if (
                        part_id not in self.part_cache
                        or self.part_cache[part_id].size != part["size"]
                    ):
...
class Partition:
    def __init__(
        ...
        # A fail-safe for unconfigured partitions, such as windows NTFS partitions.
    ...
    def detect_inner_filesystem(self, password):
        ...
        try:
            with luks2(
                self, "luksloop", password, auto_unmount=True
            ) as unlocked_device:
    ...
    def has_content(self):
        ...
        temporary_mountpoint = (
            "/tmp/"
            + hashlib.md5(bytes(f"{time.time()}", "UTF-8") + os.urandom(12)).hexdigest()
        )
    ...
    def format(
        ...
        if filesystem == "btrfs":
            ...
            if b"UUID" not in o:
                raise DiskError(
                    f"Could not format {path} with {filesystem} because: {o}"
                )
...
class Filesystem:
    ...
    def __enter__(self, *args, **kwargs):
        if self.blockdevice.keep_partitions is False:
            ...
            if self.mode == GPT:
                if (
                    self.raw_parted(f"{self.blockdevice.device} mklabel gpt").exit_code
                    == 0
                ):
        ...
        # TODO: partition_table_type is hardcoded to GPT at the moment. This has to be changed.
            ...
            log(
                f"Kept partition format {self.mode} for {self.blockdevice}",
                level=logging.DEBUG,
            )
        else:
            raise DiskError(
    ...
    def find_partition(self, mountpoint):
        for partition in self.blockdevice:
            if (
                ...
                or partition.mountpoint == mountpoint
            ):
                return partition
    ...
    def add_partition(self, type, start, end, format=None):
        ...
        if partitioning:
            ...
            while previous_partitions == self.blockdevice.partitions:
                # Let the new partition come up in the kernel
                time.sleep(0.025)
