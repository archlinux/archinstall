...
def print_large_list(options, padding=5, margin_bottom=0, separator=": "):
    ...
    if len(options) > max_options_in_cells:
        ...
        for row in range(0, (get_terminal_height() - margin_bottom)):
            for column in range(
                row, len(options), (get_terminal_height() - margin_bottom)
            ):
...
def ask_for_superuser_account(
    prompt="Username for required super-user with sudo privileges: ", forced=False
):
    while 1:
...
def ask_for_additional_users(
    ...
    while 1:
        ...
        if input("Should this user be a sudo (super) user (y/N): ").strip(
            " "
        ).lower() in ("y", "yes"):
...
def ask_to_configure_network():
    ...
    if nic and nic != "Copy ISO network configuration to installation":
        ...
        if mode == "IP":
            while 1:
                ip = input(
                    f"Enter the IP and subnet for {nic} (example: 192.168.0.5/24): "
                ).strip()
