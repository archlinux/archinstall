...
def print_large_list(options, padding=5, margin_bottom=0, separator=': '):
	...
	longest_line = (
		highest_index_number_length + len(separator) + get_longest_option(options) + padding
	)
	...
	if len(options) > max_options_in_cells:
		...
		for row in range(0, (get_terminal_height() - margin_bottom)):
			for column in range(row, len(options), (get_terminal_height() - margin_bottom)):
				...
				print(
					f"{str(column): >{highest_index_number_length}}{separator}{options[column]}",
					end=spaces,
				)
			print()
...
def generic_multi_select(
	options,
	text="Select one or more of the options above (leave blank to continue): ",
	sort=True,
	default=None,
	allow_empty=False,
):
...
def ask_for_superuser_account(
	prompt='Username for required superuser with sudo privileges: ', forced=False
):
	while 1:
...
def ask_for_additional_users(
	prompt='Any additional users to install (leave blank for no users): ',
):
	users = {}
	superusers = {}
	...
	while 1:
		...
		if input("Should this user be a superuser (sudoer) [y/N]: ").strip(' ').lower() in (
			'y',
			'yes',
		):
...
def ask_for_a_timezone():
	while True:
		timezone = (
			input(
				'Enter a valid timezone (examples: Europe/Stockholm, US/Eastern) or press enter to use UTC: '
			)
			.strip()
			.strip('*.')
		)
		...
		if (pathlib.Path("/usr") / "share" / "zoneinfo" / timezone).exists():
			...
			log(
				f"Specified timezone {timezone} does not exist.", level=logging.WARNING, fg='red'
			)
...
def ask_for_bootloader() -> str:
	...
	if hasUEFI() == False:
		...
		bootloader_choice = input(
			"Would you like to use GRUB as a bootloader instead of systemd-boot? [y/N] "
		).lower()
...
def ask_for_audio_selection():
	...
	pipewire_choice = input(
		"Would you like to install pipewire instead of pulseaudio as the default audio server? [Y/n] "
	).lower()
...
def ask_to_configure_network():
	...
	if nic and nic != 'Copy ISO network configuration to installation':
		...
		mode = generic_select(
			['DHCP', 'IP'],
			f"Select which mode to configure for {nic} or leave blank for DHCP: ",
		...
		if mode == 'IP':
			while 1:
				...
				try:
					...
					log(
						"You need to enter a valid IP in IP-config mode.", level=logging.WARNING, fg='red'
					)
...
def ask_for_disk_layout():
	...
	value = generic_select(
		options,
		"Found partitions on the selected drive, (select by number) what you want to do: ",
...
def generic_select(
	options,
	input_text="Select one of the above by index or absolute value: ",
	...
	if type(options) not in [list, dict]:
		log(
			f" * Generic select doesn't support ({type(options)}) as type of options * ",
			fg='red',
		)
		log(
			" * If problem persists, please create an issue on https://github.com/archlinux/archinstall/issues * ",
			fg='yellow',
		)
...
def select_disk(dict_o_disks):
	...
	if len(drives) >= 1:
		...
		log(
			f"You can skip selecting a drive and partitioning and use whatever drive-setup is mounted at /mnt (experimental)",
			fg="yellow",
		)
		drive = generic_select(
			drives,
			'Select one of the above disks (by name or number) or leave blank to use /mnt: ',
			options_output=False,
		)
		if not drive:
			return drive
			...
			if not selected_language:
				...
				while True:
					...
					if len(new_options) <= 0:
						log(
							f"Search string '{filter_string}' yielded no results, please try another search.",
							fg='yellow',
						)
						continue
...
def select_driver(options=AVAILABLE_GFX_DRIVERS):
	...
	if drivers:
		...
		initial_option = generic_select(
			drivers, input_text="Select your graphics card driver: "
		)
		...
		if type(selected_driver) == dict:
			...
			driver_package_group = generic_select(
				driver_options,
...
def select_kernel(options):
	...
	if kernels:
		return generic_multi_select(
			kernels,
			f"Choose which kernel to use (leave blank for default: {DEFAULT_KERNEL}): ",
