...
class luks2:
    ...
    def encrypt(
        ...
        try:
            ...
            if err.exit_code == 256:
                ...
                devinfo = json.loads(
                    b"".join(sys_command(f"lsblk --fs -J {partition.path}")).decode(
                        "UTF-8"
                    )
                )["blockdevices"][0]
                ...
                if len(children := devinfo.get("children", [])):
                    for child in children:
                        ...
                        log(
                            f"Closing crypt device {child['name']}", level=logging.DEBUG
                        )
        ...
        if cmd_handle.exit_code != 0:
            raise DiskError(
                f'Could not encrypt volume "{partition.path}": {cmd_output}'
            )
    ...
    def unlock(self, partition, mountpoint, key_file):
        ...
        while (
            pathlib.Path(partition.path).exists() is False
            and time.time() - wait_timer < 10
        ):
            time.sleep(0.025)
