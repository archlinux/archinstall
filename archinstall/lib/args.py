...
class ArchConfigHandler:
    ...
    def _fetch_from_url(self, url: str) -> str:
        if urllib.parse.urlparse(url).scheme:
            try:
                ...
                # FIXED: Added a 15-second timeout to prevent infinite hanging
    ...
    # FIXED: Added safe handling to support both ArgumentParser Namespace objects AND dictionaries,
    # as calling .items() on a Namespace object without using `vars()` throws an AttributeError.
    def _cleanup_config(self, config: Namespace | dict[str, Any]) -> dict[str, Any]:
        clean_args = {}
        # Convert Namespace to dict if necessary before iterating
