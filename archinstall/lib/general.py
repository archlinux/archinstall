        ...
        if isinstance(obj, dict):
            ...
            for key, val in list(obj.items()):
                if isinstance(val, dict):
                    val = json.loads(
                        json.dumps(val, cls=JSON)
                    )  # This, is a EXTREMELY ugly hack..
...
class sys_command:  # Thread):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if kwargs["emulate"]:
            self.log(
                f"Starting command '{cmd}' in emulation mode.", level=logging.DEBUG
            )
        ...
        if workdir := kwargs.get("workdir", None):
            ...
            self.cwd = (
                f"{user_catalogue}/.cache/archinstall/workers/{kwargs['worker_id']}/"
            )
    ...
    def run(self):
        ...
        if not self.pid:  # Child process
            ...
            if not self.kwargs["emulate"]:
                try:
                    os.execve(
                        self.cmd[0], self.cmd, {**os.environ, **self.environment_vars}
                    )
        ...
        while alive and not self.kwargs["emulate"]:
            for fileno, event in poller.poll(0.1):
                ...
                if "debug" in self.kwargs and self.kwargs["debug"] and len(output):
                    self.log(
                        self.cmd, "gave:", output.decode("UTF-8"), level=logging.DEBUG
                    )
                ...
                if "events" in self.kwargs:
                    ...
                    if len(self.kwargs["events"]) == 0:
                        ...
                        if (
                            ...
                            if "debug" in self.kwargs and self.kwargs["debug"]:
                                self.log(
                                    f"{self.cmd[0]} has finished.", level=logging.DEBUG
                                )
                            alive = False
                            break
        ...
        if "debug" in self.kwargs and self.kwargs["debug"]:
            self.log(
                f"{self.cmd[0]} got exit code: {self.exit_code}", level=logging.DEBUG
            )
